...
class Widget extends React.Component {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'outpatientpayment/getHisList',
      payload: {
        pageSize: 10000,
        pageIndex: 1,
      },
    ...
    dispatch({
      type: 'outpatientpayment/getServiceType',
      payload: {
  ...
  serviceSubInfo = (serviceTypeDate) =>{
    ...
    dispatch({
      type: 'outpatientpayment/getSubServiceType',
      payload: {
      ...
      <div className={styles['p-version']}>
        <Form className={styles.query}>
          <Row>
            ...
            <Col span={10}>
              <Form.Item label="姓名/就诊卡号" {...layout}>
                <Input
...
function mapState(state) {
  const { outpatientpayment = {} } = state;
  return {
    modelData: outpatientpayment,
  };
}
